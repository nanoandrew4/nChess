...
if [[ ! -d "build/profiling/" ]]
then
    echo "Creating profiling target directory at build/"
...
cmake -DCMAKE_BUILD_TYPE=Profiling -DENABLE_PGO=Off -DCMAKE_C_COMPILER=${C_COMPILER} -DCMAKE_CXX_COMPILER=${CXX_COMPILER} -G "CodeBlocks - Unix Makefiles" ../../
